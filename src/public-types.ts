...
export type DisplayReference = Readonly<{
  ...
  artifact?:
    | `@ripota/parks/boundaries/${string}.geojson`
    | `@ripota/parks/v3/boundaries/${string}.geojson`;
}>;
...
export type GeoJsonFeatureCollection = Readonly<{
  ...
  features: readonly Readonly<{
    ...
    geometry: Readonly<{ type: string; coordinates: Coordinates }>;
  }>[];
}>;
export type CatalogRecord = Readonly<{
  ...
  locationDesc: string;
  potaUrl: string;
  mapPoint?: Readonly<{ latitude: number; longitude: number; notes: string }>;
  status: ReviewStatus;
  geometryKind: GeometryKind;
  source: Readonly<{
    name: string;
...
export type V3CatalogRecord = Omit<CatalogRecord, "source"> &
  (
    | Readonly<{
        ...
        source?: never;
        provenance: Readonly<{ kind: "official-pota-coordinate"; url: string }>;
      }>
    | Readonly<{
        status: "available" | "point-only";
        fidelity: "reviewed-display";
        source: CatalogRecord["source"];
        provenance: Readonly<{ kind: "reviewed-source"; url: string }>;
      }>
  );
export type V3Catalog = Omit<Catalog, "references" | "schemaVersion"> &
  Readonly<{ schemaVersion: 3; references: readonly V3CatalogRecord[] }>;
