...
- **Derivation:** the route is buffered by 30.48 meters following the [POTA trail activation guidance](https://docs.pota.app/docs/activator_reference/activator_guide-english.html#special-considerations-for-trails). The output is explicitly labeled `activation-zone`, not `boundary`.
- **Limitations:** The approximation uses a local planar projection and overlapping segment/cap polygons; it is a visualization of the reviewed rule and route snapshot, not a legal or official POTA geometry.
